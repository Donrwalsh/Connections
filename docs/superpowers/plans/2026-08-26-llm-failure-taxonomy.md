...
- The `unclassified` catch-all and `groupCountOff` checks only run when `parseGroupsSection`'s structured `### GROUPS`-block parse found at least one group (i.e. when `parsedGroupWords.length > 0`, the same condition that already decides whether `sourceGroups` uses `parsedGroupWords` over `fallbackGroups`). A total fallback to the `### ANSWER` block has no per-group visibility to check and is out of scope.
- `unclassified` is scoped to the group numbers the response's own `Group N` headings actually mention (1 through the highest heading number seen), never against the puzzle's total remaining group count. The model normally addresses just one group per call — that's the common case, not an error — so comparing against how many groups the *puzzle* still needs would misfire on every ordinary single-group response.
