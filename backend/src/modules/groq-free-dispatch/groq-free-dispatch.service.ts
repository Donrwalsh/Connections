...
/**
 * The Groq counterpart to GoogleFreeDispatchService: a self-rescheduling tick
 * chain that dispatches llm-groq trials against unrun puzzles until every
 * configured Groq model is RPD-held (see RateLimitHoldService) or out of
 * unrun puzzles. Like Google, there is no per-token free budget — Groq
 * enforces a per-day request cap of its own, so "keep dispatching until
 * held" is the whole stop condition. Reuses the OPENAI_FREE_TIER_DISPATCH_*
 * pacing knobs rather than introducing a parallel env family (see
 * docs/superpowers/specs/2026-09-04-daily-free-tier-automation-design.md and
...
export class GroqFreeDispatchService {
  ...
  async start(): Promise<{
    ...
    if (allExhausted) {
      ...
      this.logger.log(
        "groq free-tier dispatch: every model is already RPD-held — not starting a cycle",
      );
  ...
  async runTick(): Promise<void> {
    ...
    if (exhausted.size === eligibleModels.length) {
      await this.stateRepo.update({ id: GROQ_DISPATCH_STATE_ID }, { active: false });
      this.logger.log(
        "groq free-tier dispatch: ran out of unrun puzzles for every eligible model — stopping",
      );
      return;
    }
