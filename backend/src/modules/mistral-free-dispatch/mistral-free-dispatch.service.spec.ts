...
describe("MistralFreeDispatchService", () => {
  let service: MistralFreeDispatchService;
  ...
  const zeroCounts = () => new Map(MISTRAL_MODELS.map((model) => [model, 0]));
  ...
  beforeEach(async () => {
    ...
    mockSupportedModelService = {
      findModelNamesByStrategy: jest.fn().mockResolvedValue([...MISTRAL_MODELS]),
    };
    mockHoldService = { heldModels: jest.fn().mockResolvedValue([]) };

    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MistralFreeDispatchService,
        { provide: getRepositoryToken(MistralDispatchState), useValue: mockStateRepo },
    ...
    service = module.get<MistralFreeDispatchService>(MistralFreeDispatchService);
  });
  ...
  describe("start", () => {
    it("should reject starting a cycle that's already active", async () => {
      mockStateRepo.findOne.mockResolvedValueOnce({ id: "mistral", active: true });
    ...
    it("should not start a cycle, and report alreadyExhausted, when every model is held", async () => {
      ...
      mockHoldService.heldModels.mockResolvedValueOnce([...MISTRAL_MODELS]);
      ...
      expect(mockStateRepo.save).toHaveBeenCalledWith(
        expect.objectContaining({ id: "mistral", active: false }),
      );
    });
    ...
    it("should start a cycle and queue the first tick when at least one model is free", async () => {
      ...
      mockHoldService.heldModels.mockResolvedValueOnce(["ministral-8b-latest"]);

      const result = await service.start();

      expect(result.outcome).toBe("started");
      expect(mockStateRepo.save).toHaveBeenCalledWith(
        expect.objectContaining({ id: "mistral", active: true }),
      );
      expect(mockQueue.add).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          delay: 0,
          jobId: expect.stringContaining("mistral-free-dispatch-"),
        }),
      );
    });
  });
  ...
  describe("stop", () => {
    it("should deactivate the cycle and return its status", async () => {
      mockStateRepo.findOne.mockResolvedValueOnce({ id: "mistral", active: false, startedAt: null });

      const result = await service.stop();

      expect(mockStateRepo.update).toHaveBeenCalledWith({ id: "mistral" }, { active: false });
  ...
  describe("runTick", () => {
    ...
    it("should stop when no Mistral models are configured", async () => {
      mockStateRepo.findOne.mockResolvedValueOnce({ id: "mistral", active: true });
    ...
    it("should hold off dispatching, but keep ticking, once the in-flight backlog hits its cap", async () => {
      ...
      inFlight.set("mistral-small-latest", 3);
      mockStateRepo.findOne.mockResolvedValueOnce({ id: "mistral", active: true });
    ...
    it("should dispatch only to eligible (non-held) models, least-allocated first", async () => {
      ...
      mockStateRepo.findOne.mockResolvedValueOnce({ id: "mistral", active: true });
      mockHoldService.heldModels.mockResolvedValueOnce(["mistral-small-latest"]);
      mockStrategyService.countTodayDispatchByModel.mockResolvedValueOnce(
        new Map([["ministral-8b-latest", 0]]),
      );
