...
describe("SambaNovaRpdResumeService", () => {
  let service: SambaNovaRpdResumeService;
  ...
  beforeEach(async () => {
    ...
    service = module.get(SambaNovaRpdResumeService);
  ...
  it("revives parked runs whose model is no longer held and re-enqueues them", async () => {
    ...
    strategyRunRepo.find.mockResolvedValue([
      parkedRun({ id: 1, puzzleId: 10, trialNumber: 0, modelName: "DeepSeek-V3.1", puzzle: { date: "2026-01-01" } }),
      parkedRun({ id: 2, puzzleId: 11, trialNumber: 1, modelName: "gpt-oss-120b", puzzle: { date: "2026-01-02" } }),
    ]);
    ...
    expect(queue.add).toHaveBeenCalledWith(
      ...
      { jobId: expect.stringMatching(new RegExp(`^${runStrategyJobId(10, "llm-sambanova", 0)}-resume-`)) },
    );
    expect(result).toMatchObject({ cleared: ["DeepSeek-V3.1"], redispatched: 1 });
  });
  ...
  it("re-arms a delayed sweep at the soonest live resetAt when parked runs remain, capped at 15 minutes", async () => {
    holdService.heldModels.mockResolvedValue(["DeepSeek-V3.1"]);
    holdService.nextResetAt.mockResolvedValue(new Date(Date.now() + 5 * 60_000));
    strategyRunRepo.find.mockResolvedValue([
      parkedRun({ id: 1, puzzleId: 10, trialNumber: 0, puzzle: { date: "2026-01-01" } }),
    ]);
