...
describe("MistralRateLimitHoldService", () => {
  let service: MistralRateLimitHoldService;
  let repo: {
    upsert: jest.Mock;
    findOne: jest.Mock;
    find: jest.Mock;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(MistralRateLimitHold), useValue: repo },
      ],
    }).compile();

    service = module.get(MistralRateLimitHoldService);
  });
  ...
  it("upserts a hold row keyed on (strategyName, modelName) with resetAt = heldAt + resetInSeconds", async () => {
    ...
    await service.hold("llm-mistral", "mistral-small-latest", 21600);
    const after = Date.now();

    expect(repo.upsert).toHaveBeenCalledTimes(1);
    const [row, conflictPaths] = repo.upsert.mock.calls[0];
    expect(row).toMatchObject({ strategyName: "llm-mistral", modelName: "mistral-small-latest" });
    const deltaMs = row.resetAt.getTime() - row.heldAt.getTime();
    expect(deltaMs).toBe(21600 * 1000);
  ...
  it("isHeld is true only while resetAt is in the future", async () => {
    ...
    expect(await service.isHeld("llm-mistral", "m")).toBe(true);

    repo.findOne.mockResolvedValueOnce({ resetAt: new Date(Date.now() - 60_000) });
    expect(await service.isHeld("llm-mistral", "m")).toBe(false);
  ...
  it("heldModels queries for future resetAt and returns the model names", async () => {
    ...
    const result = await service.heldModels("llm-mistral");

    expect(result).toEqual(["a", "b"]);
    expect(repo.find).toHaveBeenCalledWith({
      where: { strategyName: "llm-mistral", resetAt: MoreThan(expect.any(Date)) },
    });
  });
  ...
  it("nextResetAt returns the soonest still-future resetAt, or null when nothing is held", async () => {
    ...
    expect(await service.nextResetAt("llm-mistral")).toEqual(soon);
    expect(repo.find).toHaveBeenCalledWith({
      where: { strategyName: "llm-mistral", resetAt: MoreThan(expect.any(Date)) },
    });

    repo.find.mockResolvedValueOnce([]);
    expect(await service.nextResetAt("llm-mistral")).toBeNull();
  });
