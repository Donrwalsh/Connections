...
You must set at least `INTERNAL_API_KEY` (any shared secret) and `OPENAI_API_KEY` (for AI Assist). `GOOGLE_API_KEY` is only required if you plan to dispatch `llm-google` runs, and `GROQ_API_KEY` only for `llm-groq` runs. All variables are documented in `.env.sample`.
...
| `GROQ_MODEL` | `llama-3.1-8b-instant` | Groq model id (used by the `llm-groq` strategy and provider-less requests) |
...
| `LLM_GROQ_CONCURRENCY` | `1` | Maximum `llm-groq` runs the worker processes at once (own queue, so it never blocks OpenAI, Ollama, Google, or the deterministic strategies) |
...
| `LLM_GROQ_RATE_LIMIT_FALLBACK_SECONDS` | `60` | Fallback wait before retrying a Groq rate-limit hit, used only when Groq's own `x-ratelimit-*` headers don't specify one. A per-minute hit waits and retries indefinitely; a daily-quota hit (the per-day request cap) parks the model until UTC midnight |
